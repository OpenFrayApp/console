 ...
 *
 * A formula the dice engine won't take falls back to the printed average too. opendice is
 * properly strict — nonsense, a thousand dice, a total past the exact-integer range — but it
 * says so by throwing, and this is called while a creature is being added to the board, where
 * a throw is a broken add rather than a bad number. A stat block that came from a shared
 * encounter or a pasted import is the case that makes that reachable.
 */
