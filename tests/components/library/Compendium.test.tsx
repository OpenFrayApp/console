...
const loaders = vi.hoisted(() => ({
  loadLibraries: vi.fn(),
  loadSrdCreatures: vi.fn(),
  loadSrdSpells: vi.fn(),
}))

vi.mock('../../../src/compendium/srd.ts', () => loaders)
...
const { Compendium } = await import('../../../src/components/library/Compendium.tsx')

beforeEach(() => {
  loaders.loadLibraries.mockResolvedValue({ creatures, spells })
  loaders.loadSrdCreatures.mockResolvedValue(creatures)
  loaders.loadSrdSpells.mockResolvedValue(spells)
})
...
describe('Compendium', () => {
  ...
  it('loads only enabled libraries for the first creature search', async () => {
    render(<Compendium onCreateCreature={() => {}} enabledLibraries={['srd-5.2']} />)

    await waitFor(() => expect(screen.getByText('Goblin')).toBeInTheDocument())
    expect(loaders.loadLibraries).toHaveBeenCalledWith(['srd-5.2'])
    expect(loaders.loadSrdCreatures).not.toHaveBeenCalled()
    expect(loaders.loadSrdSpells).toHaveBeenCalledTimes(1)
  })
